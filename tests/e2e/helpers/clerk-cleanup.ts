...
// The mark must sit in the local part of the address (before the @).
const TEST_MARK_IN_LOCAL_PART = /^[^@]*\+clerk_test[^@]*@/i;
...
export function shouldDeleteClerkUser(userEmails: string[], recordedEmail: string): boolean {
  ...
  return emails.every((e) => TEST_MARK_IN_LOCAL_PART.test(e)) && emails.includes(recordedEmail.toLowerCase());
}
...
export async function deleteRecordedClerkUsers(): Promise<void> {
  // Never throws: globalSetup/globalTeardown must not fail the suite over cleanup.
  try {
    const file = recordFile();
    ...
    if (secretKey.startsWith('sk_live_')) {
      console.warn('Refusing to delete e2e users on a live Clerk instance.');
      return;
    }
    ...
    console.warn('Clerk cleanup failed:', error);
  }
}
